...
def compute_knn_accuracy(x, y):
    ...
    acc1 = np.mean(cross_validate(
        estimator=NN, cv=LeaveOneOut(), X=x, y=y.ravel())["test_score"])
    NN = KNeighborsClassifier(2, algorithm="brute")
    acc2 = np.mean(cross_validate(
        estimator=NN, cv=LeaveOneOut(), X=x, y=y.ravel())["test_score"])
    NN = KNeighborsClassifier(3, algorithm="brute")
    acc3 = np.mean(cross_validate(
        estimator=NN, cv=LeaveOneOut(), X=x, y=y.ravel())["test_score"])
...
def process_epochs(run, architecture):
    ...
    for filename in sorted_nicely(os.listdir(run)):
        if filename.endswith(".pt"):
            print("========", filename, "========")
            eval(pretrained_weights=os.path.join(
                run, filename), architecture=architecture)
            print("")
...
def eval(pretrained_weights, architecture):
    ...
    dataset_eval = formula_data.SingleData(
        name="weak_eval2", is_processed=True)
    dataloader_eval = torch.utils.data.DataLoader(
        dataset_eval, batch_size=32, shuffle=False)
    ...
    discrete_triples_loss, hinge_triples_loss = compute_triples_loss(
        X, y, sample=True)
