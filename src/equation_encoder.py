...
class NetSmall(nn.Module):
    ...
    def forward2(self, in1, in2):
        ...
        if self.with_dot_product:
            result = torch.bmm(out1.view(-1, 1, 64),
                               out2.view(-1, 64, 1)).view(-1)
            return result
    ...
    def forward3(self, in1, in2, in3):
        ...
        if self.with_dot_product:
            dist_sim = torch.bmm(out1.view(-1, 1, 64),
                                 out2.view(-1, 64, 1)).view(-1)
            dist_dissim = torch.bmm(
                out1.view(-1, 1, 64), out3.view(-1, 64, 1)).view(-1)
            dist_dissim2 = torch.bmm(
                out2.view(-1, 1, 64), out3.view(-1, 64, 1)).view(-1)
    ...
    def load_checkpoint(self, epoch):
        ...
        self.load_state_dict(torch.load(
            self.checkpoint_string.format(epoch), map_location='cpu'))
...
class NetLarge(NetSmall):
    ...
    def kill_batch_norm(self):
        if self.use_batch_norm:
            ...
            self.fc4.weight.data = torch.matmul(self.fc4.weight.data, torch.diag(
                1.0 / torch.sqrt(self.lin_norm.running_var + self.lin_norm.eps)))
            self.fc4.bias.data -= torch.matmul(
                self.fc4.weight.data, self.lin_norm.running_mean)
...
def print_histogram(dist, remainder="\b", compute_max=None, compute_min=None):
    ...
    try:
        dist = dist.cpu()
        hist_max = compute_max if compute_max is not None else torch.max(
            dist).item()
        hist_min = compute_min if compute_min is not None else torch.min(
            dist).item()
        hist = torch.histc(dist.cpu(), bins=15, min=hist_min,
                           max=hist_max).detach().numpy()
        m = np.max(hist)
        ...
        for i, data in enumerate(trainloader):
            if triples:
                ...
                if with_dot_product:
                    ...
                    loss = criterion(dist_sim, dist_dissim,
                                     ones[:in1.shape[0]])
                    ...
                    if i % loss_output_interval == loss_output_interval - 1:
                        # print(dist_sim)
                        # print(dist_dissim)
                        print_histogram(dist_sim.detach(),
                                        compute_max=1, compute_min=-1)
                        print_histogram(dist_dissim.detach(), loss.item(
                        ), compute_max=1, compute_min=-1)
                else:
                    out1, out2, out3 = net.forward3(in1, in2, in3)

                    dist_sim = euclid(out1, out2)
                    dist_dissim = torch.min(
                        euclid(out1, out3), euclid(out2, out3))
                    loss = torch.max(zeros[:in1.shape[0]],
                                     1.0 - dist_dissim / (dist_sim + 1.0))
                    loss = torch.mean(loss)
                    ...
                    if i % loss_output_interval == loss_output_interval - 1:
                        ...
                        print_histogram(dist_dissim, loss.item(),
                                        compute_max=2, compute_min=0)
            else:
                in1, in2, labels = data
                in1 = in1.to(device)
                in2 = in2.to(device)
                ...
                if with_dot_product:
                    ...
                    if i % loss_output_interval == loss_output_interval - 1:
                        print_histogram(
                            labels * dist, compute_max=1, compute_min=-1)
                else:
                    loss = criterion(dist, labels)
                    if i % loss_output_interval == loss_output_interval - 1:
                        print_histogram((labels - 0.5) * dist,
                                        compute_max=1, compute_min=-1)
